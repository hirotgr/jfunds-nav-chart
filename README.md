...
* 正規化中は価額チャートの下にドローダウンチャートを表示します。基準日以降の累積最高値からの下落率を、価額チャートと同じ色で比較できます。
  * 基準日を0として計算を開始し、基準日前は計算・描画しません。
  * 価額チャートと表示期間、ズーム、パン、クロスヘアを同期します。
...
* チャートの右下をつかんで動かせばチャートサイズを変更できます。チャート操作はTradingViewに準拠しています。
  * ドローダウンチャートは右下のつまみで高さだけを変更でき、横幅は価額チャートに追従します。
* 正規化中のPNG保存では、価額チャートとドローダウンチャートを1枚の画像にまとめます。
...
* Lightweight Charts™︎ 4.1.7 (©TradingView) ライブラリ取得のために CDN `https://cdn.jsdelivr.net/` への接続性が必要です
...
version 0.2.0 で正規化を行った際に基準日以降のドローダウンチャートを描画するようにしました。
